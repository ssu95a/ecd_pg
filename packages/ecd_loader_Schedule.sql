CREATE OR REPLACE PACKAGE ECD_loader_Schedule
   ...
   DECLARE
      ...
      cPkg_Name CONSTANT varchar(30 ) := 'ECD_loader_Schedule';
...
$function$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.get_Agr_Sign_Date';
   l_dt_Sign date;
BEGIN
   ...
   CALL ECD_loader_Log.dbg(
      cProc,
      'enter',
      'agr_Id=' || coalesce(p_agr_Id::varchar, '<NULL>')
   );
   ...
   CALL ECD_loader_Log.dbg(
      cProc,
      'exit',
      'dt_Sign=' || coalesce(l_dt_Sign::varchar, '<NULL>')
   );
...
$procedure$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.load_Schedule_Debt';
...
BEGIN
   ...
   l_dt_Sign := ECD_loader_Schedule.get_Agr_Sign_Date(p_agr_Id);

   CALL ECD_loader_Log.dbg(
      cProc,
      'var_val',
      'Agr_Sign_Date=' || coalesce(l_dt_Sign::varchar, '<NULL>')
   );
      ...
      SELECT x.nPart,
        ...
        FROM XMLTABLE(
           ...
           COLUMNS
              ...
              xItem  xml     PATH 'CDA_SCHEDULE/schedule_debt'
        ) x
   LOOP
      CALL ECD_loader_Log.dbg(
         cProc,
         'part loop',
         'part=' || coalesce(r.nPart::varchar, '<NULL>')
         || ', count=' || coalesce(r.nCount::varchar, '<NULL>')
      );
      ...
      SELECT
       ...
       WHERE to_date(i.pay_Date_S, 'YYYY-MM-DD') >= l_dt_Sign;
      ...
      CALL ECD_loader_Log.dbg(
         ...
         'part result',
         'part=' || coalesce(r.nPart::varchar, '<NULL>')
         || ', dt_End=' || coalesce(l_dt_End::varchar, '<NULL>')
      );
      ...
      IF l_dt_End IS NOT NULL THEN
         CALL ecd_loader_Dep.update_History(
            ...
            p_c_Val  => to_char(l_dt_End, 'DD.MM.YYYY'),
...
$procedure$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.load_Schedule_Cred';
   r record;
BEGIN
   CALL ECD_loader_Log.inf(
      cProc,
      ...
      SELECT x.nPart,
        ...
        FROM XMLTABLE(
           ...
           COLUMNS
              ...
              xItem  xml     PATH 'CDA_SCHEDULE/schedule_cred'
        ) x
   LOOP
      CALL ECD_loader_Log.dbg(
         cProc,
         'part loop',
         'part=' || coalesce(r.nPart::varchar, '<NULL>')
         || ', isProl=' || coalesce(r.isProl::varchar, '<NULL>')
         || ', count=' || coalesce(r.nCount::varchar, '<NULL>')
      );
...
$procedure$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.load_Schedule_Percent';
   l_d_From      date;
   l_b_Date      date;
...
BEGIN
   ...
   IF ECD_loader_Xml.exists_Node(
      ...
      CALL ECD_loader_Log.dbg(
         cProc,
         'before global schedule_percent',
         '//CDA_SCHEDULE/schedule_percent/item'
      );
      ...
      CALL ECD_loader_Log.dbg(
         cProc,
         'var_val',
         'd_From=' || coalesce(l_d_From::varchar, '<NULL>')
      );
         ...
         SELECT x.nPart,
           ...
           FROM XMLTABLE(
              ...
              COLUMNS
                 ...
                 xItem  xml     PATH 'CDA_SCHEDULE/schedule_adj_percent'
           ) x
      LOOP
         CALL ECD_loader_Log.dbg(
            cProc,
         ...
         SELECT x.nPart,
           ...
           FROM XMLTABLE(
              ...
              COLUMNS
                 ...
                 xItem  xml     PATH 'CDA_SCHEDULE/schedule_dfr_percent'
           ) x
      LOOP
         CALL ECD_loader_Log.dbg(
            cProc,
         ...
         SELECT x.nPart,
           ...
           FROM XMLTABLE(
              ...
              COLUMNS
                 ...
                 xItem  xml     PATH 'CDA_SCHEDULE/schedule_csn_percent'
           ) x
      LOOP
         CALL ECD_loader_Log.dbg(
            cProc,
   ...
   IF p_ctx.correct_schedule_percent THEN
      IF l_d_From IS NOT NULL THEN
         CALL ECD_loader_Log.dbg(
            cProc,
            'before CorrSum_PC',
            'd_From=' || l_d_From::varchar
         );
   ...
   CALL ECD_loader_Log.dbg(
      cProc,
      'var_val',
      'cession_buy=' || coalesce(l_b_Date::varchar, '<NULL>')
   );

   IF l_b_Date IS NOT NULL THEN
      CALL ECD_loader_Log.dbg(
         cProc,
         'before Corr_IntCalcSum',
         'b_Date=' || l_b_Date::varchar
      );
...
$procedure$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.load_Schedule_Payment';
   r record;
BEGIN
   CALL ECD_loader_Log.inf(
      cProc,
      ...
      SELECT x.nPart,
        ...
        FROM XMLTABLE(
           ...
           COLUMNS
              ...
              xItem  xml     PATH 'CDA_SCHEDULE/schedule_payment'
        ) x
   LOOP
      CALL ECD_loader_Log.dbg(
         cProc,
...
$procedure$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.load_Schedule_Overdue';
   r record;
BEGIN
   CALL ECD_loader_Log.inf(
      cProc,
      ...
      SELECT x.nPart,
        ...
        FROM XMLTABLE(
           ...
           COLUMNS
              ...
              xItem  xml     PATH 'CDA_SCHEDULE/schedule_overdue'
        ) x
   LOOP
      CALL ECD_loader_Log.dbg(
         cProc,
      ...
      DELETE
        FROM CD_Cdo_Ces
       WHERE nCdoCesAgrID = p_Agr_Id
         AND iCdoCesPart  = r.nPart;

      INSERT INTO cd_cdo_ces(
         nCdoCesAgrID,
         iCdoCesPart,
         dCdoCesStart,
         dCdoCesDate,
      ...
      SELECT
         p_agr_Id,
         r.nPart,
         to_date(i.dtBeg_S, 'YYYY-MM-DD'),
         to_date(i.dtChange_S, 'YYYY-MM-DD'),
         coalesce(ECD_loader_Xml.to_Money(i.overdue_Sum_S), 0),
         i.overdue_Type
        FROM XMLTABLE(
           'item'
           PASSING r.xItem
...
$procedure$
   #package
DECLARE
   cProc CONSTANT varchar := cPkg_Name || '.load_All';
BEGIN
   CALL ECD_loader_Log.inf(
      cProc,
      'enter',
   ...
   CALL ECD_loader_Log.dbg(cProc, 'before load_Schedule_Debt', NULL::varchar);
   CALL ECD_loader_Schedule.load_Schedule_Debt(p_agr_Id, p_xml);

   CALL ECD_loader_Log.dbg(cProc, 'before load_Schedule_Cred', NULL::varchar);
   CALL ECD_loader_Schedule.load_Schedule_Cred(p_agr_Id, p_xml);

   CALL ECD_loader_Log.dbg(cProc, 'before load_Schedule_Percent', NULL::varchar);
   CALL ECD_loader_Schedule.load_Schedule_Percent(p_ctx, p_agr_Id, p_xml);

   CALL ECD_loader_Log.dbg(cProc, 'before load_Schedule_Payment', NULL::varchar);
   CALL ECD_loader_Schedule.load_Schedule_Payment(p_agr_Id, p_xml);

   CALL ECD_loader_Log.dbg(cProc, 'before load_Schedule_Overdue', NULL::varchar);
   CALL ECD_loader_Schedule.load_Schedule_Overdue(p_agr_Id, p_xml);
